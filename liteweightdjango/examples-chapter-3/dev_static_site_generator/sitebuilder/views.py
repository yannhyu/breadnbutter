...
def get_page_or_404(name):
    ...
    try:
        # Uses safe_join to join the page’s file path with 
        # the template’s filename and returns a normalized, 
        ...
        if not os.path.exists(file_path):
            raise Http404('pagee Not Found')

    with open(file_path, 'r') as f:
        # Opens up each file and instantiates a new 
        # Django template object with its contents
...
def page(request, slug='index'):
    ...
    # Passes the page and slug context to be rendered 
    # by the page.html layout template
