...
rs = (grequests.post(EB_REAL_TIME_URL, verify=True, data=xml_req, headers=HEADERS) for xml_req in xml_inputs)

resp_payloads = grequests.map(rs)

for payload in resp_payloads:
    print(prettify(payload.content))
