...
class Typed(object):
    ...
    def __init__(self, name=None):    # Make name arg optional
        self.name = name    
...
def validate(**kwargs):    # Generate code such as: "name = String().. shares = Integer()"
    def decorate(cls):
        for name, val in kwargs.items():
            setattr(cls, name, val(name))    # Reflect the pattern: "price = Integer('price')"
        return cls
    return decorate


@validate(name=String, shares=Integer, price=Float)    # Use class decorator to assist class definition
